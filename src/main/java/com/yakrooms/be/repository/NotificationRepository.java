...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Modifying
    @Query("DELETE FROM Notification n WHERE n.booking.room.hotel.id = :hotelId")
    void deleteByHotelIdInBatch(@Param("hotelId") Long hotelId);
    List<Notification> findByUser(User user);
    List<Notification> findByUserAndIsReadFalse(User user);
    
    @Query("SELECT n FROM Notification n LEFT JOIN FETCH n.user LEFT JOIN FETCH n.booking LEFT JOIN FETCH n.booking.room WHERE n.user = :user")
    List<Notification> findByUserWithAssociations(@Param("user") User user);
    
    @Query("SELECT n FROM Notification n LEFT JOIN FETCH n.user LEFT JOIN FETCH n.booking LEFT JOIN FETCH n.booking.room WHERE n.user.id = :userId AND n.isRead = false")
    List<Notification> findByUserAndIsReadFalseWithAssociations(@Param("userId") Long userId);
    
    @Query("SELECT n FROM Notification n LEFT JOIN FETCH n.user LEFT JOIN FETCH n.booking LEFT JOIN FETCH n.booking.room WHERE n.user.id = :userId")
